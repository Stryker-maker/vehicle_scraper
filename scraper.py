...
# Self-managing location thresholds
CANDIDATE_THRESHOLD  = 3   # appearances before a city is added
ZERO_WEEK_THRESHOLD  = 8   # consecutive empty runs before a city is removed
CITY_STATS_FILE      = "data/city_stats.json"
...
# ─────────────────────────────────────────────
def load_city_stats():
    """Load persistent city stats from disk, or return empty structure."""
    if os.path.exists(CITY_STATS_FILE):
        try:
            ...
            pass
    return {
        "candidates": {},   # city -> appearance count (not yet in search list)
        "zero_weeks": {}    # city -> consecutive weeks with zero matches
    }


def save_city_stats(stats):
    """Persist city stats to disk."""
...
def normalize_city(city_raw):
    """Normalize city string to 'Title Case, XX' format."""
...
def update_locations(matches):
    """
    Core self-managing location logic.
    - Tracks cities appearing in filtered results that aren't in search_locations
    - Promotes candidates to search_locations after CANDIDATE_THRESHOLD appearances
    - Removes cities from search_locations after ZERO_WEEK_THRESHOLD consecutive empty runs
    - Logs all changes clearly
    Updates config.json and city_stats.json in place.
    """
        ...
        if count >= CANDIDATE_THRESHOLD:
            ...
            if coords:
                ...
                if dist <= max_distance_km:
                    ...
                    print(f"  [AUTO-ADD] '{city}' added to search list "
                          f"(appeared {count} times, {dist} km away)")
                else:
                    print(f"  [SKIPPED]  '{city}' hit {count} appearances "
                          f"but is {dist} km away — outside range")
                    del candidates[city]
            else:
                print(f"  [SKIPPED]  '{city}' could not be geocoded — not added")
                del candidates[city]

    # ── Step 3: update zero-week counters for active search cities ────────
    ...
    for city in list(search_locations):
        ...
        if norm in cities_with_matches:
            # Had results this run — reset counter
            zero_weeks.pop(norm, None)
        else:
            # No results this run — increment counter
            ...
            if zero_weeks[norm] >= ZERO_WEEK_THRESHOLD:
                ...
                removed.append(city)
                del zero_weeks[norm]
                print(f"  [AUTO-REMOVE] '{city}' removed after "
                      f"{ZERO_WEEK_THRESHOLD} consecutive weeks with no matches")

    # ── Step 4: save updated config.json and city_stats.json ─────────────
    if promoted or removed:
        cfg["search_locations"] = search_locations
        with open("config.json", "w") as f:
            json.dump(cfg, f, indent=2)
        print(f"  config.json updated — "
              f"{len(search_locations)} cities now in search list")
    ...
    print(f"\n  Location system summary:")
    print(f"    Active search cities:   {len(search_locations)}")
    print(f"    Candidate cities:       {len(candidates)}")
    if candidates:
        for city, count in sorted(candidates.items(),
                                  key=lambda x: x[1], reverse=True):
            print(f"      {city}: {count}/{CANDIDATE_THRESHOLD} appearances")
    if zero_weeks:
        print(f"    Cities on watch (no recent matches):")
        for city, weeks in sorted(zero_weeks.items(),
                                  key=lambda x: x[1], reverse=True):
            print(f"      {city}: {weeks}/{ZERO_WEEK_THRESHOLD} empty weeks")
        ...
        try:
            ...
            for item in raw_listings:
                try:
                    ...
                    # Mileage - comes as "155,773 km", strip to integer
                    ...
                    # Location - address lives in location object, not seller
                    ...
                    # Build full dealer address from location fields
                    dealer_address = ""
                    if loc_street:
...
def rank_listings(matches):
    ...
    # Ensure mileage is always an integer
    for v in matches:
        try:
