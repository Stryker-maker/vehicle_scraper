...
def _candidate_is_eligible(*, candidate: Any, current: dict[str, Any]) -> bool:
    ...
    current_sources = [e for e in current.get("sources", []) if isinstance(e, dict)]
    if not current_sources:
        return False
    current_keys = [_source_key(e) for e in current_sources]
    if len(current_keys) != len(set(current_keys)):
        return False

    candidate_entries = [e for e in candidate.get("sources", []) if isinstance(e, dict)]
    candidate_keys = [_source_key(e) for e in candidate_entries]
    if len(candidate_keys) != len(set(candidate_keys)):
        return False
    candidate_sources = dict(zip(candidate_keys, candidate_entries))

    for current_entry in current_sources:
        key = _source_key(current_entry)
        current_fp = _compatibility_fingerprint(current_entry)
        candidate_entry = candidate_sources.get(key)
        if candidate_entry is None:
            return False
        candidate_fp = _compatibility_fingerprint(candidate_entry)
        if current_fp is None or candidate_fp is None or current_fp != candidate_fp:
            return False
    return True
...
def compare_health_reports(*, baseline: dict[str, Any] | None, current: dict[str, Any],
    ...
    """Compare current health against an eligible baseline without crossing compatibility boundaries."""
    ...
    if baseline_selection and baseline_selection.get("status") == "incompatible":
        baseline_status = "incompatible"
    elif not baseline or not isinstance(baseline.get("sources"), list):
    ...
    return _perform_comparison(baseline=baseline, current=current, run_id=run_id,
                               baseline_status=baseline_status, anomalies=anomalies)
...
def _perform_comparison(*, baseline: dict[str, Any] | None, current: dict[str, Any],
    ...
    baseline_sources = {_source_key(e): e for e in (baseline or {}).get("sources", []) if isinstance(e, dict)}
    current_sources = [e for e in current.get("sources", []) if isinstance(e, dict)]
    ...
    if baseline_status == "incompatible":
        anomalies.append(_anomaly(
            severity="info", code="baseline_incompatible", vehicle_key="", source="",
    ...
    for entry in current_sources:
        ...
        for metric, minimum, warning_ratio, critical_ratio in (("accepted_record_count", 10, 0.50, 0.25), ("fetched_record_count", 20, 0.50, 0.25)):
            old = _number(previous.get(metric)); new = _number(entry.get(metric))
            if old >= minimum:
                ratio = new / old
        ...
        old_attempts = _number(previous.get("request_attempt_count")); new_attempts = _number(entry.get("request_attempt_count"))
        ...
        old_warnings = _number(previous.get("quality_warning_rows")); new_warnings = _number(entry.get("quality_warning_rows"))
        if new_warnings >= max(5, old_warnings * 2 + 1):
            anomalies.append(_anomaly(severity="warning", code="quality_warning_growth", vehicle_key=vehicle_key, source=source,
                                      message="Rows with quality warnings increased materially.", baseline=old_warnings, current=new_warnings,
    ...
    return {"anomaly_schema_version": ANOMALY_SCHEMA_VERSION, "run_id": run_id, "generated_at_utc": utc_now(),
            ...
            "incompatible_source_count": incompatible_source_count, "anomaly_status": status,
...
def parser() -> argparse.ArgumentParser:
    ...
    build = sub.add_parser("build"); build.add_argument("--baseline", required=True); build.add_argument("--current", required=True); build.add_argument("--run-id", required=True)
    check = sub.add_parser("check"); check.add_argument("--report", default="data/run_status/anomalies_latest.json"); check.add_argument("--policy", choices=ANOMALY_POLICIES, required=True)
    return result
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.action == "build":
        baseline = load_optional_json(Path(args.baseline)); current = load_optional_json(Path(args.current))
        if current is None: raise SystemExit("Current health report is missing or invalid")
        ...
        print(json.dumps({"report": report, "artifacts": [str(path) for path in paths]}, indent=2, sort_keys=True)); return 0
    if args.action == "check":
        ...
        if report is None or report.get("anomaly_schema_version") != ANOMALY_SCHEMA_VERSION:
            print("Anomaly report is missing or invalid"); return 1
        print(json.dumps(report, indent=2, sort_keys=True))
        if args.policy == "enforce" and int(report.get("critical_anomaly_count", 0)) > 0: return 1
        return 0
