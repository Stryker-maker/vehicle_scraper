...
class BaselineCompatibilityFingerprintTests(unittest.TestCase):
    """Verify deterministic construction of the baseline compatibility contract."""

    def setUp(self):
        """Build a representative vehicle configuration used by the fingerprint tests."""
        self.config = {
            "schema_version": 2,
            ...
            "sources": {
                "autotrader": {
                    "make": "subaru",
                    "model": "forester",
                    "search_locations": ["Edmonton, AB", "Calgary, AB", "Saskatoon, SK", "Regina, SK"],
                },
                "kijiji": {
                    "make": "Subaru",
                    "model": "Forester",
    ...
    def test_identity_contains_contract_fields(self):
        """Ensure the identity exposes every required compatibility dimension."""
        identity = build_compatibility_identity(config=self.config, source="kijiji", collection_scope="full", adapter_schema_version=1)
    ...
    def test_location_order_does_not_change_fingerprint(self):
        """Verify semantic location ordering is independent of input order."""
        _, first = build_compatibility_fingerprint(config=self.config, source="kijiji", collection_scope="full", adapter_schema_version=1)
        reordered = copy.deepcopy(self.config)
        reordered["sources"]["kijiji"]["search_locations"] = list(reversed(reordered["sources"]["kijiji"]["search_locations"]))
        _, second = build_compatibility_fingerprint(config=reordered, source="kijiji", collection_scope="full", adapter_schema_version=1)
    ...
    def test_adding_a_location_changes_fingerprint(self):
        """Verify adding a collection location invalidates the compatibility fingerprint."""
        _, baseline = build_compatibility_fingerprint(config=self.config, source="kijiji", collection_scope="full", adapter_schema_version=1)
        changed = copy.deepcopy(self.config)
        changed["sources"]["kijiji"]["search_locations"].append("Kelowna, BC")
        _, current = build_compatibility_fingerprint(config=changed, source="kijiji", collection_scope="full", adapter_schema_version=1)
        self.assertNotEqual(baseline, current)

    def test_removing_a_location_changes_fingerprint(self):
        """Verify removing a collection location invalidates the compatibility fingerprint."""
        _, baseline = build_compatibility_fingerprint(config=self.config, source="kijiji", collection_scope="full", adapter_schema_version=1)
        changed = copy.deepcopy(self.config)
    ...
    def test_population_affecting_configuration_changes_fingerprint(self):
        """Verify a population-affecting configuration change invalidates the fingerprint."""
        _, baseline = build_compatibility_fingerprint(config=self.config, source="kijiji", collection_scope="full", adapter_schema_version=1)
        changed = copy.deepcopy(self.config)
    ...
    def test_scope_and_versions_change_fingerprint(self):
        """Verify collection scope and semantic schema versions are compatibility boundaries."""
        _, baseline = build_compatibility_fingerprint(config=self.config, source="kijiji", collection_scope="full", adapter_schema_version=1)
        _, single_pair = build_compatibility_fingerprint(config=self.config, source="kijiji", collection_scope="single_pair", adapter_schema_version=1)
        self.assertNotEqual(baseline, single_pair)
        identity = build_compatibility_identity(config=self.config, source="kijiji", collection_scope="full", adapter_schema_version=1)
        changed_version = copy.deepcopy(identity)
        changed_version["adapter_schema_version"] = 2
        self.assertNotEqual(compatibility_fingerprint(identity), compatibility_fingerprint(changed_version))

    def test_autotrader_does_not_depend_on_kijiji_registry_version(self):
        """Verify AutoTrader identities omit the Kijiji-specific location registry version."""
        identity = build_compatibility_identity(config=self.config, source="autotrader", collection_scope="full", adapter_schema_version=1)
        self.assertIsNone(identity["location_registry_version"])

    def test_unsupported_source_is_rejected(self):
        """Verify unknown sources cannot produce a compatibility identity."""
        with self.assertRaisesRegex(ValueError, "Unsupported source"):
            build_compatibility_identity(config=self.config, source="unknown", collection_scope="full", adapter_schema_version=1)
